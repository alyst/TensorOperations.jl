...
#-----------------------------------------
facts("@tensor macro") do
A=randn((3,5,4,6))
p=randperm(4)
C1=permutedims(A,p)
#@eval @tensor C2[$(p...)] := A[1,2,3,4] # FIXME @eval does not work inside fact()
#@fact C2 --> roughly(C1, atol=eps()*sqrt(length(C1))*vecnorm(C1+C2))
@fact_throws TensorOperations.IndexError begin
...
@fact_throws DimensionMismatch begin
...
@fact D1 --> roughly(D2, atol=eps()*sqrt(length(D1))*vecnorm(D1+D2))
@fact vecnorm(D1) --> roughly(sqrt(abs(@tensor scalar(D1[d,f,h]*conj(D1[d,f,h])))))
...
@fact C --> roughly(Ccopy, atol=eps(Float32)*vecnorm(C+Ccopy)*sqrt(length(C)))
Cbig=rand(Complex64,(40,40,40))
C=sub(Cbig,3+2*(0:8),13+(0:8),7+3*(0:7))
Ccopy=copy(C)
